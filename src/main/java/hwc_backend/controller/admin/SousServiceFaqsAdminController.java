...
@RestController
@RequestMapping("/api/admin/sous-service-faqs")
@RequiredArgsConstructor
public class SousServiceFaqsAdminController {

    private final SousServiceFaqsService service;

    @GetMapping
    public ResponseEntity<Page<SousServiceFaqsDTO>> getAll(Pageable pageable) {
    ...
    @PostMapping
    public ResponseEntity<SousServiceFaqsDTO> create(@Valid @RequestBody SousServiceFaqsDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(dto));
    }

    @PutMapping("/{id}")
    public ResponseEntity<SousServiceFaqsDTO> update(@PathVariable Long id, @Valid @RequestBody SousServiceFaqsDTO dto) {
