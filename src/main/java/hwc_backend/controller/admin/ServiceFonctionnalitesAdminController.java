...
@RestController
@RequestMapping("/api/admin/service-fonctionnalites")
@RequiredArgsConstructor
public class ServiceFonctionnalitesAdminController {

    private final ServiceFonctionnalitesService service;

    @GetMapping
    public ResponseEntity<Page<ServiceFonctionnalitesDTO>> getAll(Pageable pageable) {
    ...
    @PostMapping
    public ResponseEntity<ServiceFonctionnalitesDTO> create(@Valid @RequestBody ServiceFonctionnalitesDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(dto));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ServiceFonctionnalitesDTO> update(@PathVariable Long id, @Valid @RequestBody ServiceFonctionnalitesDTO dto) {
