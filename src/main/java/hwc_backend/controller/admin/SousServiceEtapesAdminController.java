...
@RestController
@RequestMapping("/api/admin/sous-service-etapes")
@RequiredArgsConstructor
public class SousServiceEtapesAdminController {

    private final SousServiceEtapesService service;

    @GetMapping
    public ResponseEntity<Page<SousServiceEtapesDTO>> getAll(Pageable pageable) {
    ...
    @PostMapping
    public ResponseEntity<SousServiceEtapesDTO> create(@Valid @RequestBody SousServiceEtapesDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(dto));
    }

    @PutMapping("/{id}")
    public ResponseEntity<SousServiceEtapesDTO> update(@PathVariable Long id, @Valid @RequestBody SousServiceEtapesDTO dto) {
