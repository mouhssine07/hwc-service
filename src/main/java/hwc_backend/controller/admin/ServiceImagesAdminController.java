...
@RestController
@RequestMapping("/api/admin/service-images")
@RequiredArgsConstructor
public class ServiceImagesAdminController {

    private final ServiceImagesService service;

    @GetMapping
    public ResponseEntity<Page<ServiceImagesDTO>> getAll(Pageable pageable) {
    ...
    @PostMapping
    public ResponseEntity<ServiceImagesDTO> create(@Valid @RequestBody ServiceImagesDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(dto));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ServiceImagesDTO> update(@PathVariable Long id, @Valid @RequestBody ServiceImagesDTO dto) {
